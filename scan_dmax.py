...
# ---------------------------------------------------------------------------
q_min = 0.2  # nm⁻¹  (0 = no lower cut)
q_max = 4.0  # nm⁻¹  (practical upper limit; override if data quality allows)
...
# Dmax scan
# ---------------------------------------------------------------------------
dmax_values = np.arange(6.5, 20.5, 0.5)  # nm
results = []

print(
    f"{'Dmax':>6}  {'conv':>5}  {'iter':>5}  {'alpha':>8}  "
...
for Dmax in dmax_values:
    res = solve_pr(
        q, I_obs, sigma, Dmax, n_r=60, tol=0.01, icf_width=icf_width
    )
    results.append((Dmax, res))
    chi2_sc = res.chi2 / res.c2
    print(
        f"{Dmax:6.1f}  {str(res.converged):>5}  {res.iterations:5d}  "
...
F_fit = (
    4 * np.pi * np.trapezoid(best_res.pr[None, :] * sinc, best_res.r, axis=1)
)
ax.errorbar(
    q, I_obs, yerr=sigma, fmt="k.", ms=2, alpha=0.3, label="data", zorder=1
)
ax.plot(
    q, F_fit, "r-", lw=1.5, label=f"fit  Dmax={best_dmax:.1f} nm", zorder=2
)
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlabel("q  (nm⁻¹)")
