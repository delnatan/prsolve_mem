...
from prsolve import solve_pr, sample_pr
...
# ---------------------------------------------------------------------------
q_min = 0.05  # nm⁻¹  (0 = no lower cut)
q_max = 4.5    # nm⁻¹  (practical upper limit; override if data quality allows)
...
print(f"ICF width  : {icf_width:.3f} nm  (= π / 2q_max, q_max = {q.max():.3f} nm⁻¹)")
...
print(f"chi²/c²   : {result.chi2/c2:.2f}  (scaled, should ≈ N−G = {N - result.G:.1f})")
...
samples = sample_pr(q, I_obs, sigma, result, n_samples=500)
lo, hi = np.percentile(samples, [5, 95], axis=0)

fig, axes = plt.subplots(1, 2, figsize=(11, 4))

ax = axes[0]
ax.fill_between(result.r, lo, hi, color="steelblue", alpha=0.25, label="90% band")
...
axes[1].errorbar(q, I_obs, yerr=sigma, fmt="k.", ms=2, alpha=0.4, label="data", zorder=1)
